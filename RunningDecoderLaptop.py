...
class LT_FullHybridDecoderNew(object):
    ...
    def __init__(
        ...
        gstate,
        strats,
        measurement_order=[x for x in range(1, 8)],
        first_output_qbt=0,
        in_qubit=0,
        ...
        if self.printing:
            for key in self.anti_support_dict_hybrid.keys():
                print(
                    ...
                    key,
                    ", anticom. support ",
                    self.anti_support_dict_hybrid[key],
                )
    ...
    def filter_strat_lost_qubit(self, strats, lost_qbt_ix):
        ...
        return [
            these_stabs for these_stabs in strats if these_stabs[3][lost_qbt_ix] == "I"
        ]
    ...
    def decode_hybrid(self, input_strats):
        ...
        while len(current_poss_strats) != 0:
            ...
            loss_patts = self.decode_specific_output_qbt(
                poss_further_strats[:-1], output_qbt, tracking_lost_output_qbts
            )
            ...
            current_poss_strats = self.filter_strat_lost_qubit(
                current_poss_strats, output_qbt
            )
            self.measurement_order.remove(self.measurement_order[0])
            if self.printing:
                print("Just measured output qubit: ", output_qbt)
                print(
                    "Number of tolerable loss patterns so far is ",
                    len(analytic_exp),
                    ", and number of surviving strategies is ",
                    len(current_poss_strats),
                )
            if len(current_poss_strats) != 0:
                current_poss_strats.sort(
                    key=lambda x: x[4]
                )  # Do I need to do thsi again ?
    ...
    def get_next_strat(
        self, strats, meas_before_qbt_and_pauli, next_measure_qbt, previous_output_qbt=0
    ):
        meas_before = []
        meas_pauli = {}
    ...
    def get_next_strat_version2(
        self, strats, meas_before_qbt_and_pauli, previous_output_qbt=0
    ):
        meas_before = []
        meas_pauli = {}
        for outcome in meas_before_qbt_and_pauli:
    ...
    def decode_specific_output_qbt(self, input_strats, output_qbt, lost_output_qbt=[]):
        ...
        while lt_finished:
            ...
            for key in current_keys:
                discovered_qbts = []
                meas_order = [
                    tree_dict[key][2][idx] for idx in range(len(tree_dict[key][2]))
                ]
                # meas_qbt = [output_qbt]
                meas_qbt = []
                meas_before = [
                    tree_dict[key][1][idx][0] for idx in range(len(tree_dict[key][1]))
                ]
                ...
                starting_stab = self.get_next_strat(
                    tree_dict[key][0], tree_dict[key][1], next_meas_qbt, output_qbt
                )
                ...
                tree_dict = self.update_tree_meas_pattern(
                    meas_qbt, starting_stab, tree_dict, key
                )
                tree_dict = self.update_identity_in_tree_meas_pattern(
                    meas_qbt, starting_stab, tree_dict, key
                )
                ...
                if (
                    pot_new_output_qbt != output_qbt
                    and pot_new_output_qbt not in meas_qbt
                    and pot_new_output_qbt != next_meas_qbt
                ):
                    starting_qbts_init.append(pot_new_output_qbt)
                starting_qbts_init_extras = starting_stab[
                    1
                ]  # Only considers the supp of the attempted qbt
                ...
                for qbt in self.measurement_order:
                    if (
                        qbt not in meas_before
                        and qbt not in lost_qbts
                        and qbt not in lost_output_qbt
                    ):
                        starting_qbts.append(qbt)
                    else:
                        discovered_qbts.append(qbt)
                succes_measured_stabs[key] = [
                    starting_stab,
                    copy.deepcopy(meas_qbt),
                    meas_order,
                    copy.deepcopy(lost_output_qbt),
                    starting_qbts,
                ]
                ...
                for qbt in starting_qbts:
                    ...
                    if self.no_anti_com_flag:
                        ...
                        update_strats = self.filter_anticom_strats_TBH(
                            update_strats, copy.deepcopy(meas_qbt)
                        )
                    else:
                        if len(update_strats) < 2:
                            ...
                            update_strats = self.update_valid_strategies(
                                self.in_qubit, output_qbt, key, qbt, update_strats
                            )
                            save_key = key + "," + str(qbt)
                    ...
                    tree_dict[save_key] = [
                        copy.deepcopy(update_strats),
                        copy.deepcopy(meas_qbt),
                        copy.deepcopy(meas_order),
                    ]
                    if len(update_strats) > 1:
                        # Continue the loop, there are more valid strategies
                        new_keys.append(save_key)
                    elif len(update_strats) == 1:
                        leaf_meas_qbt = self.update_meas_qbts_in_loop(
                            copy.deepcopy(meas_qbt), update_strats[0]
                        )
                        leaf_meas_qbt = self.update_identity_in_meas_qbts_in_loop(
                            leaf_meas_qbt, update_strats[0]
                        )
                        succes_measured_stabs[save_key] = [
                            update_strats,
                            leaf_meas_qbt,
                            copy.deepcopy(meas_order),
                            copy.deepcopy(lost_output_qbt),
                        ...
                        # meas_qbt = self.update_meas_qbts_in_loop(
                        #     meas_qbt, update_strats[0]
                        # )
                        # meas_qbt = self.update_identity_in_meas_qbts_in_loop(
                        #     meas_qbt, update_strats[0]
                        # )
                        # succes_measured_stabs[save_key] = [
                        #     update_strats,
                        ...
                        lost_qbts = self.from_key_get_list_of_lost_qbt(
                            key + "," + str(qbt)
                        )
        ...
        for key in succes_measured_stabs.keys():
            ...
            self.get_anticommuting_support(
                succ_patt,
                anti_support,
                list_of_anti_support,
                this_succ_patt_output_qbt,
                output_qbt,
            )
    ...
    def update_identity_in_tree_meas_pattern(
        self, meas_qbts, new_strat, tree, current_key
    ):
        new_pauli_measurement_qubits = new_strat[1]
        new_pauli_measurements = new_strat[3]
        new_pauli_meas = []
    ...
    def update_valid_strategies(self, in_qubit, output_qbt, key, qbt, in_strats):
        ...
        for qbt_out in range(len(self.gstate)):
            if (
                qbt_out != in_qubit
                and qbt_out != output_qbt
                and qbt_out in list(self.output_qbt_counter.keys())
            ):
                ...
                for l_qbt in lost_qbts:
                    poss_further_strats = self.filter_strat_lost_qubit(
                        poss_further_strats, l_qbt
                    )
    ...
    def get_anticommuting_support_old(
        self,
        outcome_list,
        anti_support,
        list_of_anti_support,
    ...
    def calculate_log_transmisison_from_min_loss_patt(self, transmission):
        ...
        for trans in transmission:
            ...
            for n_qbt in range(min_qbts_lost + 1):
                ...
                tot_trans += pref * (
                    ((1 - trans) ** n_qbt) * (trans ** (n_total - n_qbt))
                )
            log_trans.append(tot_trans)
        return log_trans
...
def search_for_meas_patterns(n_qbts=6):
    ...
    gstate = graph_from_nodes_and_edges(
        graph_nodes, graph_edges
    )  # graphstate_from_nodes_and_edges(graph_nodes, graph_edges)
    ...
    for meas_pattern in patterns:
        ...
        adaptive_decoder_new_no_extra_matt = LT_FullHybridDecoderNew(
            copy.deepcopy(gstate),
            copy.deepcopy(input_strats),
            measurement_order=copy.deepcopy(meas_pattern_list),
            no_anti_com_flag=True,
            printing=False,
        )
...
def search_for_meas_patterns_cube(n_qbts=7):
    ...
    for idx_meas, meas_pattern in enumerate(patterns):
        ...
        meas_pattern_list = [qbt for qbt in meas_pattern]
        adaptive_decoder_new = LT_FullHybridDecoderNew(
            copy.deepcopy(gstate),
            copy.deepcopy(input_strats),
            measurement_order=copy.deepcopy(meas_pattern_list),
            no_anti_com_flag=False,
        ...
        tree, node_info, name_idx, leaf_idxs = get_tree_fixed_meas_patt(
            adaptive_decoder_new.tree_branches
        )
        max_cd, all_leaves = compute_max_capture_delay(
            tree, node_info, name_idx, leaf_idxs, printing=False
        )

        branching_peak, all_leaves, _ = compute_branching_capture_delay(
            tree, node_info, name_idx, leaf_idxs, all_leaves, printing=False
        )
...
def loop_large_graphs(
    ...
    #   ## CURRENT DECODER erasure_decoder = LT_Erasure_decoder(n_qbts, distance, gstate, in_qbt=in_qubit)
    input_strats = erasure_decoder.strategies_ordered
    measurement_pattners = get_full_m_patt_list(filename)
    print("Number of measurement patterns: ", len(measurement_pattners))

    if (idxMin == None) or (idxMax == None):
        idxMin = 0
        idxMax = len(measurement_pattners)

    matter_qbts_saved = []
    los_tol_list = []

    for m_idx in range(idxMin, idxMax):
        meas_pattern_list = measurement_pattners[m_idx]

        t1 = time.time()
        print("%d -- %d -- %d" % (idxMin, m_idx, idxMax), end=", ")
        print("Meas_Pattern_list: ", meas_pattern_list)
        adaptive_decoder_new = LT_FullHybridDecoderNew(
            copy.deepcopy(gstate),
            copy.deepcopy(input_strats),
            measurement_order=copy.deepcopy(meas_pattern_list),
        ...
        matt_qbts, all_leaves, _ = compute_branching_capture_delay(
            tree, node_info, name_idx, leaf_idxs, all_leaves, printing=False
        )
        ...
        # print("At idx: ", m_idx)
        print(
            "Max # loss photons: ", len(adaptive_decoder_new.all_min_loss_patterns[0])
        )
        print("Number of matter qubits: ", matt_qbts)
        dT = time.time() - t1
